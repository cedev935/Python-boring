...
> 方法五可用时间：2017-08-11
*****
> 方法六最新可用时间：2017-08-11
*****
> 方法七最新可用时间: 2017-08-11
...
然后打开百度网盘首页，按f12键，然后在console标签页里（默认的就是）输入

> document.querySelector('#export_menu').id = 'export_menu2';

PS:感谢@Runjuu 

![](https://user-images.githubusercontent.com/16254644/29202348-e181b6bc-7e99-11e7-82ce-5b4a8bf02d00.png)
...
然后会弹出如图的窗口，这个时候会出来一堆的文件链接，-o后面的文件名如果是你的文件名才是真的下载链接，别的是别的文件的下载链接。

![](https://user-images.githubusercontent.com/16254644/29202528-b1fb181e-7e9b-11e7-938a-bc12c487601d.png)

然后删除所选链接中红框框框住的这一串命令
...
**重要** 在所选链接-x16后面按一下空格添加-t1 -m0 （如图）
...
**注意：这步是关键：在所选链接把参数-s10 -k1M -x16 改成 -s256 -k2M -x256 ,不过这只是建议，如果你的网速还要快的话，建议把-s和-x改成更高，如果你改成很高了还是觉得不够快，你可以更改-k后面的参数，变成-k3M甚至-k5M，-k10M,然后在这些参数后面按一下空格加上-t1 -m0这个参数，然后变成如下图这样子的**
...
现在打开http://138.68.239.39:2333/ 这个网站
...
这个时候打开http://138.68.239.39:10000/lua/network_stats.lua （用户名密码均为download）
...
然后转到http://138.68.239.39:10000/lua/network_stats.lua （用户名密码均为download）查看下载进度，注意看这个traffic底下那栏，记数据为b，这个时候你的文件下载了多少的公式就是（b-a）G。然后还可以通过底下的仪表盘来看速度（1mbps近似取100k，以此类推）
...
这个时候请到http://138.68.239.39:80 里面的download文件夹下载你下载的文件，也可以到ftp://138.68.239.39 来下载（较慢）
...
然后解压，复制到c:\windows\system32里，然后打开http://138.68.239.39:80 右键你要下载的文件，获取链接，然后打开cmd，输入
