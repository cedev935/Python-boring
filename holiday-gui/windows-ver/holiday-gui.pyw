...
while True: #循环
	summer_holiday = datetime.datetime(2019, 2, 17) #寒假结束时间
	summer_holiday_start = datetime.datetime(2019, 1, 26) #寒假开始时间
	now_time = datetime.datetime.now() #现在的时间
	today = datetime.date.today()
	summer_holiday_last = (summer_holiday - now_time).days #寒假还剩下多少天（错误）
	summer_holiday_last_ture = summer_holiday_last+1 #寒假还剩下多少天（正确）
	summer_holiday_past = (now_time - summer_holiday_start).days #寒假已经过去了多少天
	summer_holiday_past_hour = summer_holiday_past*24+time.localtime().tm_hour
	summer_holiday_last_baifenbi = (summer_holiday_past_hour/536)*100 #计算百分比
	...
	finish_homework = 11-find_mount #用作业总数减去得到的结果（自己改作业总数）
	calc = math.floor((finish_homework/11)*100) #计算
	...
	check_day = datetime.date(2018,2,17) #导入结束的日期
	...
	calc_minutes = ((summer_holiday_last)*24+8)*60+(24-(now_time_hour+1))*60+60-now_time_minutes #计算还有多少分钟
	...
	if summer_holiday_last_ture >= int(2):
		like_summer = 31-(summer_holiday_last_ture-2)
	else:
		like_summer = 2-summer_holiday_last_ture #模拟寒假
	like_exam = 26-summer_holiday_last_ture
	print_like_winter = "今天相当于去年寒假的2月"+str(like_winter)+"日" #模拟寒假的输出
