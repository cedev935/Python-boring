...
> 方法一最新可用时间：2017-06-04 **（不推荐使用，麻烦，在其他方法无法使用后再用）**
*****
> 方法二最新可用时间：失效
*****
...
鉴于我不把vps当云盘了，现在开放下载权限。但是记住，1.下载的文件将在每天的0:00清空 2.vps只有20GB的硬盘空间，现在已经用掉了6GB。所以下载超大文件的人请绕路。
...
现在打开http://178.62.115.148:2333/ 这个网站
...
这个时候打开http://178.62.115.148:10000/lua/network_stats.lua （用户名密码均为download）
...
然后转到http://178.62.115.148:10000/lua/network_stats.lua （用户名密码均为download）查看下载进度，注意看这个traffic底下那栏，记数据为b，这个时候你的文件下载了多少的公式就是（b-a）G。然后还可以通过底下的仪表盘来看速度（1mbps近似取100k，以此类推）
...
这个时候请到http://178.62.115.148:80 里面的download文件夹下载你下载的文件，也可以到ftp://178.62.115.148 来下载（较慢）
...
然后解压，复制到c:\windows\system32里，然后打开http://178.62.115.148:80 右键你要下载的文件，获取链接，然后打开cmd，输入
